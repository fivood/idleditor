  ...
  if (list.length === 0) {
    return (
      <div className="h-full overflow-y-auto p-3 md:p-5 text-center py-10 md:py-12">
        ...
        <p className="text-muted text-[10px] md:text-xs mt-1 font-mono">当稿件出现在你的桌面上时，你会遇到作者。</p>
      </div>
    )
  }
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs md:text-sm font-bold text-ink font-mono">
                  {author.name}
                  <span className="text-[8px] md:text-[10px] text-muted font-normal ml-1.5">
                    {TIER_LABELS[author.tier] ?? author.tier}
                  </span>
                </p>
                <p className="text-[9px] md:text-xs text-muted mt-0.5 font-mono">
              ...
              {author.tier === 'new' && (
                <button
                  ...
                  className="text-[9px] md:text-xs px-2 md:px-3 py-1 md:py-1.5 bg-copper text-white border-2 border-border-dark font-mono cursor-pointer shadow-[2px_2px_0_#4a3728] active:shadow-none active:translate-x-[2px] active:translate-y-[2px] transition-all flex-shrink-0"
                >
                  签约
                </button>
              )}
  ...
  return (
    <div className="mt-1 md:mt-1.5">
      <div className="flex items-center justify-between mb-0.5">
        <span className="text-[7px] md:text-[8px] text-muted font-mono">→ {TIER_LABELS[nextTier]}</span>
