...
import { RIVALS } from '@/core/rivals'
...
export function processRandomEventPhase(world: GameWorldState, _ctx: TickContext, result: TickResult): PhaseResult {
  if (world.playTicks % 300 === 0) {
    ...
    if (randomEvt.text) {
      result.toasts.push({ id: nanoid(), text: randomEvt.text, type: 'milestone', createdAt: world.playTicks })
    }
  }
...
export function rollRandomEvent(world: GameWorldState): { world: GameWorldState; text: string | null } {
  ...
  const pool: Array<() => { world: GameWorldState; text: string | null }> = [
    () => {
      ...
      return { world, text: `🎭 一位匿名老编辑寄来了一叠修订笔记。获得 ${rp} RP。` }
    },
    () => {
      const created = createManuscriptWithWorld(world)
      world = created.world
      world.manuscripts.set(created.manuscript.id, created.manuscript)
      return { world, text: `📮 午夜投稿堆突然多了一份手稿——署名栏写着"顺路带来的"。新稿件：${created.manuscript.title}。` }
    },
    () => {
      const rp = rangeInt(3, 12)
      world.currencies.revisionPoints += rp
      return { world, text: `🦇 伯爵大人心血来潮巡视编辑部，随手批了几份稿子。获得 ${rp} RP。` }
    },
    () => ({ world, text: `☕ 茶水间的古董咖啡机今天没有爆炸。所有人精神抖擞，工作效率似乎高了一点点。` }),
    () => {
      const prestige = rangeInt(2, 8)
      world.currencies.prestige += prestige
      return { world, text: `📰 《永夜文学报》刊登了一篇关于出版社的专题报道（可能因为主编欠编辑部主任一个人情）。声望 +${prestige}。` }
    },
    () => ({ world, text: `📚 一位老顾客搬走了半座图书馆。编辑们面面相觑——"我们仓库里什么时候有这么多书？"` }),
    () => {
      const rp = rangeInt(10, 30)
      world.currencies.revisionPoints += rp
      return { world, text: `💼 版权部成功将一本书的影视改编权卖给了某流媒体平台（虽然合同细则里出现了"永久使用权"这个词）。获得 ${rp} RP。` }
    },
    () => ({ world, text: `🔔 人力资源部的新实习生不小心把咖啡洒在了档案柜上——现在所有在编作者都收到了一份"恭喜您中奖"的邮件。场面一度非常尴尬。` }),
    () => {
      const rp = rangeInt(8, 18)
      world.currencies.revisionPoints += rp
      return { world, text: `📖 一位退休教授来到编辑部，淡定地指出三本书里的历史错误，然后顺便帮你审了两本稿子。获得 ${rp} RP。` }
    },
    () => ({ world, text: `🌙 今夜是新月。年轻编辑们在办公室偷偷讲鬼故事——图书主题的鬼故事销量意外增加了。` }),
    () => {
      const rejected = [...world.authors.values()].filter(a => a.rejectedCount > 0)
      if (rejected.length === 0) return { world, text: `📬 投稿箱里出现了一封没有署名的手写信。字迹潦草，但文法不错。` }
      const a = pick(rejected)
      const snippets = [
      ...
      return { world, text: pick(snippets) }
    },
    () => {
      const rejected = [...world.authors.values()].filter(a => a.rejectedCount > 0)
      if (rejected.length === 0) return { world, text: `🗞️ 文学版编辑在报纸上写了一篇短文，标题是"为什么好稿子也会被退"。（实习生说这是他见过的最被动的攻击。）` }
      const a = pick(rejected)
      const count = a.rejectedCount
      if (count >= 3) return { world, text: `📉 ${a.name}已累计被退稿${count}次。据了解，该作家目前正在考虑转行——可能去当编辑。` }
      if (count >= 2) return { world, text: `${a.name}在接受采访时表示："永夜出版社的退稿信比我的小说还长。至少编辑认真看了。大概。"` }
      return { world, text: `${a.name}在咖啡馆偶遇编辑部主任，尴尬地聊了三分钟天气后火速离开。` }
    },
    () => {
      const rejected = [...world.authors.values()].filter(a => a.rejectedCount > 0 && a.cooldownUntil !== null)
      if (rejected.length === 0) return { world, text: null }
      const a = pick(rejected)
      const quips = [
      ...
      return { world, text: pick(quips) }
    },
    () => {
      const onCooldown = [...world.authors.values()].filter(a => a.cooldownUntil !== null && a.cooldownUntil > 0)
      if (onCooldown.length === 0) return { world, text: null }
      const a = pick(onCooldown)
      return { world, text: `⏳ ${a.name}的冷却还剩${a.cooldownUntil}秒。据线人透露，ta正在写一部"出版社不配拥有的神作"。` }
    },
    () => {
      if (world.totalPublished < 5) return { world, text: null }
      const titles = [...world.manuscripts.values()].filter(m => m.status === 'published')
      if (titles.length === 0) return { world, text: null }
      const book = pick(titles)
      const newsItems = [
      ...
      return { world, text: pick(newsItems) }
    },
    () => {
      const rp = rangeInt(15, 35)
      world.currencies.revisionPoints += rp
      return { world, text: `🏛️ 市立图书馆订购了出版社的整套目录。馆长说："预算花不完，你们懂的。"获得 ${rp} RP。` }
    },
    () => ({ world, text: `📸 一位知名书评人发了张自拍，背景里——模糊但可辨认——是出版社的大楼。配文："接下来三个月我最期待的事"。编辑们默默截图了。` }),
    () => ({ world, text: `🎂 今天是出版社的「永生茶话会」——每百年一次的团建活动。伯爵说了两句祝词，然后回棺材补觉了。` }),
    () => ({ world, text: `📬 一封寄给"永夜出版社全体员工"的匿名信。内容是一首关于破晓的十四行诗。编辑部一致同意：写得不错。但这不改变寄件人不知道我们是吸血鬼的事实。` }),
    () => {
      const poached = [...world.authors.values()].filter(a => a.poached)
      if (poached.length === 0) return { world, text: null }
      const a = pick(poached)
      const prestige = rangeInt(5, 10)
      world.currencies.prestige += prestige
      const rival = pick(RIVALS)
      return { world, text: `📰 ${a.name}在${rival.name}出版了一本新书。书评人说还不错——至少比上次被退掉那本强。作为前编辑，你获得 ${prestige} 声望。` }
    },
    () => {
      const rival = pick(RIVALS)
      const newsItems = [
