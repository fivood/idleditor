...
const SCENES: Record<string, string[]> = {
  reviewComplete: [
    ...
    '审完{title}，{editorName}把稿子放在一边，喝了一口红茶——不是你以为的那种——然后拿起下一份。三百年来日日如此。',
    '{editorName}在{title}的稿纸边缘画了一只蝙蝠。是批注。也是签名。也是"我读过了"——三种意思全占了。',
    '审{title}花的时间比预期短。不是写得短——是{editorName}看得快。当你不用呼吸的时候，阅读速度可以很快。',
    '窗外的鸽子目睹了{title}被审完的全过程。它们歪着头，似乎对结局不太满意。{editorName}对鸽子耸了耸肩。',
    '审完{title}后，{editorName}拿起铜质印章，在封面检测单上"通过"一栏敲了下去。声音在走廊里回荡了三百年。',
    '合上{title}，{editorName}对自己说：这本可以。永生者很少说"可以"——通常他们说"再等等"。但这本可以。',
  ],
  bookPublished: [
