...
const SCENES: Record<string, string[]> = {
  reviewComplete: [
    ...
    '{title}的前半部分读得{editorName}眉头紧锁——后半部分奇迹般地圆回来了。{adj_pos}。',
    '如果你觉得{title}很好——{editorName}不知道"好"这个字对于一本小说意味着什么。他已经读了太多。',
    '{title}审完。{editorName}在边缘画了一只蝙蝠。这是肯定的标志。不会有第二个人知道。',
    '审稿日记：{title}。{adj_pos}的情节，{adj_neg}的对话。{editorName}决定再喝一杯茶。',
    '{title}让{editorName}想起上世纪三十年代的某本书。具体哪本他说不上来——但那种感觉，他活了两个世纪，骗不了人。',
    '读完{title}的最后一页，{editorName}把稿子叠好放在桌上。他活了217年，还是会被某些句子戳中——虽然表面上看不出来。',
    '{editorName}审完{title}后，在办公室里来回踱了{time_minutes}步。这是他思考的方式。蝙蝠式思考。',
    '审稿批注：{title}整体{adj_pos}，第三章需要删掉四十页关于天气的描写——{editorName}已经厌倦了雨。',
    '{editorName}边读{title}边用羽毛笔在页边画小蝙蝠。这是好评的意思。也可能只是手痒。没人敢问。',
    '审{title}的时候，{editorName}的笔漏墨了。他在手稿上留下了一个不可磨灭的墨点。作者可能会以为这是隐喻。其实不是。',
  ],
  bookPublished: [
